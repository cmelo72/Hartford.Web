...
function initGallery(){
	jQuery("#wowslider-container1").wowSlider({effect:"stack",prev:"",next:"",duration:20*100,delay:20*100,outWidth:496,outHeight:196,width:496,height:196,autoPlay:true,stopOnHover:true,loop:false,bullets:true,caption:false,controls:true});
}

function initAccordion(){
	jQuery('.accordion li:has(ul)').accordion1({
		animSpeed: 500,
		activeClass: 'active',
		opener: '>a.opener',
		slide: '>ul.slide',
		...
		if(slide.length) {
			...
			btn.click(function(){
				if(currentLi.hasClass(options.activeClass)){
					slide.slideUp(options.animSpeed, function(){currentLi.removeClass(options.activeClass);});
				}else{
					currentLi.addClass(options.activeClass);
					currentLi.siblings('.' + options.activeClass).removeClass(options.activeClass).find(options.slide).slideUp(options.animSpeed);
